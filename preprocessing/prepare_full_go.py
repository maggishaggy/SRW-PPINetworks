...
def get_go_terms_relationships():
    ...
    go_bp = pd.read_csv('../data/go/BPGOfull.txt', header=None, sep=' ', names=['child', 'relation', 'parent'])
    # filter for child-parent relations
    go_bp = go_bp.loc[((go_bp['relation'] == 'is_a') | (go_bp['relation'] == 'part_of'))]

    go_mf = pd.read_csv('../data/go/MFGOfull.txt', header=None, sep=' ', names=['child', 'relation', 'parent'])
    # filter for child-parent relations
    go_mf = go_mf.loc[((go_mf['relation'] == 'is_a') | (go_mf['relation'] == 'part_of'))]

    go_cc = pd.read_csv('../data/go/CCGOfull.txt', header=None, sep=' ', names=['child', 'relation', 'parent'])
    # filter for child-parent relations
...
def get_all_go_terms(go_id, go_relations):
    ...
    :return: all go terms that are ancestors to the given go term
    :rtype: set
    """
    all_go_annotations = set()
    child_go_terms = go_relations['child'].values.tolist()
    parents_go = set()
    ...
    while len(parents_go) != 0:
        ...
        if go_id in child_go_terms:
            parents = go_relations.loc[go_relations['child'] == go_id]['parent'].values.tolist()
            parents_go.update(parents)

    return all_go_annotations


def get_protein_go_annotations(goa_file, include_parents=True):
    """ Annotates proteins from string-db with go terms

    :param goa_file: file path to GOA file with go term annotations
    :type goa_file: str
    ...
    goa_human = pd.read_csv(goa_file, sep='\t', header=None, names=names, skiprows=12)
    ...
    if include_parents:
        if os.path.exists('../data/go/go_ancestors.pkl'):
            with open('../data/go/go_ancestors.pkl', 'rb') as f:
                go_terms_with_parents = pickle.load(f)
        else:
            go_relations = get_go_terms_relationships()
            go_terms_with_parents = dict()
            print('(' + time.strftime('%c') + ')  Getting ancestors for every GO term...')
            for go_id in go_terms:
                go_terms_with_parents[go_id] = get_all_go_terms(go_id, go_relations)

            with open('../data/go/go_ancestors.pkl', 'wb') as f:
                pickle.dump(go_terms_with_parents, f, pickle.HIGHEST_PROTOCOL)
    ...
    for index, row in goa_human.iterrows():
        ...
        if include_parents:
            if protein_id not in protein_annotations:
                protein_annotations[protein_id] = go_terms_with_parents[go_id].copy()
            else:
                protein_annotations[protein_id].update(go_terms_with_parents[go_id].copy())
        else:
            if protein_id not in protein_annotations:
...
def save_go_protein_annotations(file_interactions, protein_go_annotations, write_file):
    ...
    :param protein_go_annotations: dictionary for proteins with their go terms annotations
...
def save_go_protein_annotations_with_parents():
    annotations = get_protein_go_annotations('../data/go/goa_human_t1.gaf')
...
def save_go_protein_annotations_no_parents():
    annotations = get_protein_go_annotations('../data/go/goa_human_t1.gaf', include_parents=False)
    print('(' + time.strftime('%c') + ')  Writing into HumanPPI700_GO...')
    save_go_protein_annotations('../data/human_ppi_700/HumanPPI.txt', annotations,
...
def save_new_go_protein_annotations_with_parents():
    annotations = get_protein_go_annotations('../data/go/goa_human_t2.gaf')
    print('(' + time.strftime('%c') + ')  Writing into HumanPPI700_GO...')
    save_go_protein_annotations('../data/human_ppi_700/HumanPPI.txt', annotations,
                                '../data/human_ppi_700/t2/HumanPPI_GO.txt')
    print('(' + time.strftime('%c') + ')  Writing into HumanPPI900_GO...')
    save_go_protein_annotations('../data/human_ppi_900/HumanPPI.txt', annotations,
                                '../data/human_ppi_900/t2/HumanPPI_GO.txt')


def save_new_go_protein_annotations_no_parents():
    annotations = get_protein_go_annotations('../data/go/goa_human_t2.gaf', include_parents=False)
    print('(' + time.strftime('%c') + ')  Writing into HumanPPI700_GO...')
    save_go_protein_annotations('../data/human_ppi_700/HumanPPI.txt', annotations,
                                '../data/human_ppi_700/t2/HumanPPI_GO_no_parents.txt')
    print('(' + time.strftime('%c') + ')  Writing into HumanPPI900_GO...')
    save_go_protein_annotations('../data/human_ppi_900/HumanPPI.txt', annotations,
                                '../data/human_ppi_900/t2/HumanPPI_GO_no_parents.txt')


if __name__ == '__main__':
    # save_go_protein_annotations_with_parents()
    # save_go_protein_annotations_no_parents()
    # save_new_go_protein_annotations_with_parents()
    save_new_go_protein_annotations_no_parents()
